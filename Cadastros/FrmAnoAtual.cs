...
{
    public partial class FrmAnoVirgente : Form
    {
        ...
        private void grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                txtAno.Enabled = true;
                AnoAntigo = grid.CurrentRow.Cells[1].Value.ToString();
                id = grid.CurrentRow.Cells[0].Value.ToString();
                txtAno.Text = grid.CurrentRow.Cells[1].Value.ToString();
            }
            else 
            {
                return;
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            if (Ano <= anoAnterior)
            {
                MessageBox.Show("Ops! você está tentando cadastrar um ano fora de virgencia, tente cadastrar " + Ano + " ou acima", "Cadastro de Ano", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (Ano > anoSuperior)
            {
