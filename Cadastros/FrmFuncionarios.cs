...
namespace SistemaHotel.Cadastros
{
    public partial class FrmFuncionarios : Form
    {
        ...
        {
            con.AbrirConexao();
            sql = "SELECT * FROM funcionarios WHERE nome LIKE @nome ORDER BY nome asc"; //LIKE, busca nome por aproximacao
            cmd = new MySqlCommand(sql, con.con);
        ...
        private void grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                //imagem
                if (grid.CurrentRow.Cells[7].Value != DBNull.Value) //DBNull.Value campo quem do Banco de dado
                {
                    byte[] imagem = (byte[])grid.Rows[e.RowIndex].Cells[7].Value; //criada var byte[] imagem p/ receber convetido em byte[] o que vem da grid
