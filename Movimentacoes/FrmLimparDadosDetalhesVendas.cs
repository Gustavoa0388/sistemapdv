...
{
    public partial class FrmLimparDadosDetalhesVendas : Form
    {
        ...
        public FrmLimparDadosDetalhesVendas()
        {
            InitializeComponent();
        }
        ...
        private void FrmLimparDadosDetalhesVendas_Load(object sender, EventArgs e)
        {
            LoadTheme();
        }
        //metodo BackUp
        ...
        private void LimparDados()
        {
            ...
            if (bkpFeito == "Sim")
            {
                var res = MessageBox.Show("Deseja realmente LIMPAR todos os dados Detelhes das Vendas, desde de " + dtInicial.Text + " até " + dtFinal.Text + " ?", " Sistema Hotel ", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (res == DialogResult.Yes)
                {
                //botao excluir
                con.AbrirConexao();
                sql = "DELETE FROM detalhes_venda WHERE data >= @dataInicial AND data <= @dataFinal";
                ...
                cmd.ExecuteNonQuery();
                con.FecharConexao();
                MessageBox.Show("Os Detalhes das Vendas foram limpas com sucesso!", "Sistema Hotel", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
