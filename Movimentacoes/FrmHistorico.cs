...
using System.Linq;
using System.Text;
using System.Threading; //t.SetApartmentState(ApartmentState.STA);
...
namespace SistemaHotel.Movimentacoes
{
    public partial class FrmHistorico : Form
    {
        ...
        private void FrmHistorico_Load(object sender, EventArgs e)
        {
            ...
            LimparSelecao();
            ListarData();
            //somarApagar(); //EXIBIR total a pagar reserva+produto+servico e identifica se 'Aberto' ou 'Quitado'
        ...
        private void txtDinheiro_TextChanged(object sender, EventArgs e)
        {
            ...
            Troco();            
            //calculando se há valor restante apagar
            ...
            //exibir troco zero enquanto for negativo
        ...
        private void btnQuitar_Click(object sender, EventArgs e)
        {
            ...
            //var Pagar = dinheiroQ + pixQ + cartaoQ;
            ...
            var totalPagarAvista = Double.Parse(lblApagar.Text, System.Globalization.NumberStyles.Currency); //double SEM R$
            if (Pagar > totalPagarAvista)
            {
                con.AbrirConexao(); //"UPDATE produtos SET estoque=@estoque WHERE id = @id";
                ...
                cmd.ExecuteNonQuery();
                con.FecharConexao();
                //MessageBox.Show("Pagamento de: " + Pagar.ToString("C2") + " é superior que o débito.", "AVISTA", MessageBoxButtons.OK, MessageBoxIcon.Warning);                
            }          
            if (Pagar == totalPagarAvista)
            {
                con.AbrirConexao(); //"UPDATE produtos SET estoque=@estoque WHERE id = @id";
                ...
                cmd.Parameters.AddWithValue("@valor_total", (Convert.ToDouble(valor_total) - Convert.ToDouble(txtDesconto.Text)) + Convert.ToDouble(txtTaxa.Text));
               //
                cmd.Parameters.AddWithValue("@pago", Convert.ToDouble(pago) + (dinheiroQ + pixQ + cartaoQ)); //ja pago + o q ta pagando
                cmd.Parameters.AddWithValue("@apagar", 0);
                cmd.Parameters.AddWithValue("@pix", Convert.ToDouble(txtPix.Text));
            ...
            if (Pagar < totalPagarAvista)
            {
                var vRestantes = Double.Parse(lblRestante.Text, System.Globalization.NumberStyles.Currency); //double SEM R$
                con.AbrirConexao(); //"UPDATE produtos SET estoque=@estoque WHERE id = @id";
            ...
            var valorPadrao = Double.Parse(valor_total, System.Globalization.NumberStyles.Currency); //double SEM R$
        ...
        {            
            if (e.RowIndex > -1)
            {                
                Program.idVenda = grid.CurrentRow.Cells[0].Value.ToString(); //usado para exibir relatorio   
        ...
        private void TotalizandoAberto()
        {
            ...
            foreach (DataGridViewRow row in grid.Rows)
            {
                if (row.Cells["status"].Value.ToString() == "Aberto")
                {
                    ...
                    total += Convert.ToDouble(row.Cells["apagar"].Value); //pode ser pelo indece tb
                }
            }
            totalEntradas = total;
        }
        ...
        {
            double totalPago = 0;
            //percorrendo cada linha da grid, p/ pegar e somar cada linha do array.
            foreach (DataGridViewRow row in grid.Rows)
            {
                if (row.Cells["cliente"].Value.ToString() == lblCliente.Text)
                {
                    //pegando a celular e somando
                    totalPago += Convert.ToDouble(row.Cells["pago"].Value); //pode ser pelo indece tb
                }
            }
            montante = totalPago;
        ...
        private void Troco()
        {
            ...
            {               
                var Pagando = Double.Parse(lblApagar.Text, System.Globalization.NumberStyles.Currency); //double SEM R$
