...
{
    public partial class FrmLimparDadosNovoServico : Form
    {
        ...
        public FrmLimparDadosNovoServico()
        {
            InitializeComponent();
        }
        ...
        private void FrmLimparDadosNovoServico_Load(object sender, EventArgs e)
        {
            LoadTheme();
        }
        //metodo BackUp
        ...
        private void LimparDados()
        {
            ...
            if (bkpFeito == "Sim")
            {
                var res = MessageBox.Show("Deseja realmente LIMPAR todos os dados de Novo Serviço, desde de " + dtInicial.Text + " até " + dtFinal.Text + " ?", " Sistema Hotel ", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (res == DialogResult.Yes)
                {
                //botao excluir
                con.AbrirConexao();
                sql = "DELETE FROM novo_servicos WHERE data >= @dataInicial AND data <= @dataFinal";
                ...
                cmd.ExecuteNonQuery();
                con.FecharConexao();
                MessageBox.Show("Os dados de Novo Serviço foram limpos com sucesso!", "Sistema Hotel", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
