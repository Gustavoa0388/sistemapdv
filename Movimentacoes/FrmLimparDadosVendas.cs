...
{
    public partial class FrmLimparDadosVendas : Form
    {
        ...
        private void FrmLimparDadosVendas_Load(object sender, EventArgs e)
        {
            LoadTheme();
        }
        ...
        private void LimparDados()
        {
            ...
            if (bkpFeito == "Sim")
            {
                var res = MessageBox.Show("Deseja realmente LIMPAR todos os dados de Vendas, desde de " + dtInicial.Text + " até " + dtFinal.Text + " ?", " Sistema Hotel ", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (res == DialogResult.Yes)
                {
                //botao excluir
                con.AbrirConexao();
                sql = "DELETE FROM vendas WHERE data >= @dataInicial AND data <= @dataFinal";
                ...
                cmd.ExecuteNonQuery();
                con.FecharConexao();
                MessageBox.Show("Os dados de Vendas foram limpos com sucesso!", "Sistema Hotel", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
